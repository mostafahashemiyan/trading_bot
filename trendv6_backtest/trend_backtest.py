...
Separate backtesting script for the v6 signal system.

Important design choices:
- This file does NOT call the LLM.
- It imports the exact same market analysis, rule-based strategy selection,
  strategy engines, and hard validation functions from trendv6_fixed_signal_validator.py.
- It generates signals candle-by-candle using only historical data available up to that candle.
- It then tests the signal on future candles.

Keep this file in the same folder as:
    trendv6_fixed_signal_validator.py

Run:
    python trendv6_backtest_no_llm.py
...
# How many candles the strategy is allowed to wait after entry before closing.
# Example: 48 on 1h timeframe = max 48 hours in a trade.
...
# Entry mode:
# - "signal_entry": use the exact entry_price produced by the v6 signal engine.
# - "next_open": enter at the next candle open but keep the signal SL/TP levels.
DEFAULT_ENTRY_MODE = "signal_entry"

# If both stop loss and take profit are touched in the same candle, we choose stop loss first.
# This is conservative because OHLC candles do not reveal the exact intrabar sequence.
INTRABAR_AMBIGUITY_MODE = "conservative_sl_first"

# Prevent multiple overlapping trades in selected-strategy backtest.
PREVENT_OVERLAPPING_TRADES = True

# Minimum candles needed before generating a signal.
# v6 has EMA200, plus additional warmup; 240 is safe.
...
def is_executable_signal(signal: Dict) -> bool:
    """A signal is executable only when it has all trade levels."""
    if not signal:
        return False
...
def calculate_trade_r(entry: float, stop_loss: float, exit_price: float, side: str) -> float:
    """Return trade result in R units."""
    if side == "long":
...
# ============================================================
# STRATEGY SELECTION WITHOUT LLM
# ============================================================

def select_strategy_without_llm(trend_data: Dict) -> Dict:
    """
    Use the same rule-based strategy selector already present in v6.
    This replaces the LLM in backtesting.
    """
...
def generate_selected_signal_no_llm(window_df: pd.DataFrame) -> Tuple[Dict, Dict, Dict, Dict]:
    """
    Generate exactly one selected-strategy signal using no LLM.

    Returns:
        trend_data, strategy_suggestion, validation, signal
    """
    ...
    # For backtesting, only executable and hard-valid signals can be traded.
    if not hard_validation.get("is_hard_valid"):
        # Keep the original signal status/reason, but mark it clearly for the backtest layer.
        signal = dict(signal)
...
def simulate_future_trade(
    ...
    # Re-check trade geometry after entry mode adjustment.
    if side == "long":
    ...
    for j in range(signal_index + 1, max_exit_index + 1):
        ...
        # If this is the final allowed candle, close at market close.
        if j == max_exit_index:
            exit_price = close
...
def backtest_selected_strategy_no_llm(
    ...
    """
    Backtest the v6 workflow without LLM:
    trend analysis -> rule-based strategy selection -> selected strategy signal -> hard validation -> trade simulation.

    Returns:
        trades, skipped_signals
    """
    trades = []
    skipped_signals = []
    ...
    for i in range(MIN_HISTORY_CANDLES, len(df) - 1):
        ...
        try:
            ...
            skipped_signals.append({
                "signal_index": i,
                "reason": f"signal_generation_error: {exc}",
            })
            continue
...
def backtest_each_strategy_independently(
    ...
    This tests every strategy engine independently.

    This is NOT the final execution mode. It is only for comparing strategy behavior.
    """
...
def save_backtest_outputs(symbol: str, timeframe: str, report: Dict, trades: List[Dict], skipped: List[Dict]) -> Tuple[str, str, str]:
    ensure_output_dir()
    stamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
    safe_symbol = clean_symbol_for_filename(symbol)

    json_path = os.path.join(OUTPUT_DIR, f"backtest_report_{safe_symbol}_{timeframe}_{stamp}.json")
    trades_csv_path = os.path.join(OUTPUT_DIR, f"backtest_trades_{safe_symbol}_{timeframe}_{stamp}.csv")
    skipped_csv_path = os.path.join(OUTPUT_DIR, f"backtest_skipped_{safe_symbol}_{timeframe}_{stamp}.csv")
...
def print_summary(report: Dict, json_path: str, trades_csv_path: str, skipped_csv_path: str) -> None:
    ...
    metrics = report["selected_strategy_backtest"]["metrics"]
    for key, value in metrics.items():
        print(f"{key}: {value}")

    print("--------------------")
    print("By Strategy:")
    by_strategy = report["selected_strategy_backtest"].get("by_strategy", {})
    if not by_strategy:
...
def main():
    ...
    trades, skipped_signals = backtest_selected_strategy_no_llm(
        df=df,
        target_mode=target_mode,
        entry_mode=entry_mode,
        max_hold_candles=max_hold_candles,
    ...
    report = {
        ...
        "execution_mode": "selected_strategy_no_llm_rule_based_selector",
        "target_mode": target_mode,
        "entry_mode": entry_mode,
        "max_hold_candles": max_hold_candles,
        ...
        "important_note": (
            "This backtest uses the same v6 rule and signal functions, but no LLM. "
    ...
    json_path, trades_csv_path, skipped_csv_path = save_backtest_outputs(symbol, timeframe, report, trades, skipped_signals)
